...
export function BottomNav() {
  ...
  return (
    <nav
      ...
      aria-label="Primary"
    >
      <div className="mx-auto flex max-w-lg justify-around rounded-2xl border border-white/50 bg-white/85 py-1.5 shadow-lg shadow-slate-900/8 backdrop-blur-md supports-backdrop-filter:bg-white/75 sm:max-w-xl md:max-w-2xl">
        {items.map(({ to, label, icon }) => (
          ...
          >
            <span className={`text-lg leading-none transition-transform ${location.pathname === to ? 'scale-110' : ''}`}>
              {icon}
            </span>
